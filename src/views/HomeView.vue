<template>
    <div>
        ...
        <section v-if="pendingTasks.length > 0">
            ...
            <TaskItem v-for="task in pendingTasks" :key="task.id" :task="task" @toggle="toggleTask"
                @remove="removeTask" />
        </section>

        <section v-if="completedTasks.length > 0">
            <h2 class="section-title">Concluídas ({{ completedTasks.length }})</h2>
            <TaskItem v-for="task in completedTasks" :key="task.id" :task="task" @toggle="toggleTask"
                @remove="removeTask" />
        </section>
